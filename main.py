...
# create the application window with Tk()
...
class CurrencyConverter:  # Create Class
    def __init__(self):  # Special method in python class
        ...
        Label(window, font="Helvetica 12 bold", bg="yellow", text="Amount to convert").grid(row=1, column=1,
                                                                                                  sticky=W)
        Label(window, font="Helvetica 12 bold", bg="yellow", text="Conversion Rate").grid(row=2, column=1,
                                                                                                sticky=W)
        Label(window, font="Helvetica 12 bold", bg="yellow", text="Converted Amount").grid(row=3, column=1,
                                                                                                 sticky=W)

        # StringVar() is a class from Tkinter. it is used so that you can easily monitor changes to tkinter variables
        ...
        btnConvertedAmount = Button(window, text="Convert", font="Helvetica 12 bold", bg="blue", fg="white",
                                    command=self.ConvertedAmount).grid(row=4, column=2, sticky=E)
        btnDeleteAll = Button(window, text="Clear", font="Helvetica 12 bold", bg="red", fg="white",
                              command=self.delete_all).grid(row=4, column=6, padx=25, pady=25, sticky=E)
        # PadX and PadY adds padding to the outside of the button. Create spaces between button widgets in a grid
